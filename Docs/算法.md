...
> 给定链表头结点head，如果链表长度为偶数，给链表两边都设立相同的编号，按照编号排序，
>
> 如 [5,2,6,8,6,12,35,2,6] -> 各元素对应序号为：[L1,L2,L3,L4,R1,R2,R3,R4]，
>
> 按序号排序[L1,R2,L3,R4,L4,R3,L2,R1] -> 得到最终排序好的链表[5,35,6,6,8,2,2,12]

##### 单向链表按某值划分

>将单向链表按某值 [V] 划分成左变小，中间先等、右边大的形式
>
>假设链表为 3 -> 5 -> 2 -> 6 -> 1 -> 4 -> 3 -> 7 -> 0  ; V=3
>
>则将<3的放左边 ，=3的放中间 ，大于3的放右边
>
>得最终链表为：2 -> 1 -> 0 -> 3 -> 3 -> 3 -> 5  -> 6 -> 7

> 把链表放数组里，在数组上做partition（笔试用）

```java

```

>分成大中小三部分，再把各个部分之间串起来（面试用）
...
> 统计企业所有年龄阶段的员工数量，按年龄从小到大排序，要求每位员工都参与到排序中来
...
> 非负能够用十进制的来表达的数 ，先找到数组中的最大值，统计处最大值的总位数，其他不足位数的高位补0
...
public static void radixSort(int[] arr, int L, int R, int maxBit) {
    ...
    for (int b = 1; b <= maxBit; b++) {
        ...
        for (i = R; i >= L; i--) {
            ...
            // 012取个位数2，小于等于2的数有6个，因此在help中的所占区域应该是0~5，由于从右往左遍历，012在arr中处于数组最右边，固然放在help的								 最右边：help[5]，同时count[2]位置进行递减，剩余5个
            ...
            // 实际当上面0统计消耗完应该不剩了，但是这里剩一个，原因就是我们根本不是靠这个每个位对应索引上的计数来统计具体数量的，我们能确定这个     								数应该放在help[]的具体位置就OK了
...
private static int getBit(int i, int b) {
    // 从整数 i 中提取了位于位置 b 的数字。位置是从右向左计数的，从 0 开始。如果 i 是 12345，b 是 3，则表达式的结果将是 3，因为第3位的数字是3
...
> - 不存在时间复杂度O(N*logN)、空间复杂度低于O(N),并且还稳定的基于比较的排序
> - 追求绝对的速度选快排，追求节省空间选堆排、追求稳定性选归并
...
> 例如有数组如下：[2,1,1,3,2,3]  => [1,1,2,2,3,3] ，稳定性就是指，原来第一个1排完后是不是还是排在第二个1前面，第一个2排完是不是还在第二个2前面
>
> **不适用于基础类型排序，只有在非基础类型下才有意义**
>
> - 冒泡排序【相等不交换】
> - 插入排序【从后往前对比交换，相等不交换】
> - 归并排序【相等先拷贝左组，左组全部对比完再处理右组】
